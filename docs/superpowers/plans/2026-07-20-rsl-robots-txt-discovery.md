...
- **Serverless-usage licenses via robots.txt are out of scope (v1).** The origin-served path still supports the serverless-usage short-circuit in `obtainLicenseToken` unchanged. Discovery only returns a license that has a **mintable** (server-bearing) block for the resource; a robots-referenced serverless-only license is skipped.
- **Multiple mintable providers → prefer Supertab (added in Task 3).** Live inspection of inverse.com showed its rslcollective attribution license *does* carry a `server`, so "first mintable wins" resolved the wrong provider and would leak Supertab credentials to it. Selection now prefers server-bearing blocks whose host matches the configured Supertab base (`SupertabConnect.baseUrl`), falling back to any provider only when none match — via a shared `selectMintableContent` used by both the discovery gate and the mint path. See the design spec's "Provider disambiguation" section. This is a preference, not a hard restriction (a sole non-Supertab provider is still honored).
